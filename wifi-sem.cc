...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("randomChannel", "Enable/disable channel randomness", randomChannel);
  cmd.Parse (argc,argv);
  ...
  if (randomChannel)
    {
      channel.AddPropagationLoss("ns3::RandomPropagationLossModel",
                                 "Variable",
                                 PointerValue(CreateObjectWithAttributes<UniformRandomVariable>("Min", DoubleValue(0.0),
                                                                                                "Max", DoubleValue(20.0))));
    }
